...
namespace Sabio.Web.Api.Controllers
{
    ...
    public class TempAuthApiController : BaseApiController
    {
        ...
        [HttpPost("login/{userId:int}/{userName}/{role}")]
        [AllowAnonymous]
        public async Task<ActionResult<SuccessResponse>> LoginAsync(string email, string password)
        {
            var role = new[] { "User" };
            string userName = "ckirk";
            
            await _userService.LogInTest(userName + "ckirkwood208@gmail.com", "Pass123!!", 1, role);
